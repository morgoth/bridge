...
FactoryGirl.define do
  factory :board do
    association :user_n, :factory => :user
    association :user_e, :factory => :user
    association :user_s, :factory => :user
    association :user_w, :factory => :user
    deal_id "0"
    vulnerable "NONE"
    dealer "N"
  end

  factory :board_four_passes, :parent => :board do
  ...
  factory :full_board, :parent => :board do
    deal_id 636839108127179982824423290.to_s
    dealer "N"
    after_create do |board|
      board.bids.create!(:bid => "1S",   :user => board.user_n)
      board.bids.create!(:bid => "PASS", :user => board.user_e)
